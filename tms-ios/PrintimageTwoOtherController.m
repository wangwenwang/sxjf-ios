//
//  PrintimageTwoOtherController.m
//  tms-ios
//
...
@interface PrintimageTwoOtherController ()
...
// 航班号
@property (weak, nonatomic) IBOutlet UILabel *airlineNumber;
...
@implementation PrintimageTwoOtherController
...
- (void)viewDidLoad {
    ...
    _aviationMasterNo.text = _dic[@"aviationMasterNo"];//货运单号
    
    _bsOrderQty.text = [NSString stringWithFormat:@"%@", _dic[@"bsOrderQty"]];// 件数
    
    _bsOrderWt.text = [NSString stringWithFormat:@"%@", _dic[@"bsOrderWt"]];// 重量
    
    _airlineNumber.text = _dic[@"airlineNumber"];// 航班号
    
    _startEndCity.text = _dic[@"startEndCity"];// 始发站 至 到达站
    ...
    _bsOrderQty_2.text = [NSString stringWithFormat:@"%@", _dic[@"bsOrderQty"]];// 件数
    _bsOrderWt_2.text = [NSString stringWithFormat:@"%@", _dic[@"bsOrderWt"]];// 重量
    _productNo_s_label_2.text = _productNo_s;// 标签号
}
