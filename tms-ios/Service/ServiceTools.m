    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        while (1) {
            ...
            if([Tools getServerAddress]) {
                ...
                NSString *url = [NSString stringWithFormat:@"%@%@", [Tools getServerAddress], @"queryAppVersionq122.do"];
                ...
                [manager POST:url parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
                    ...
                    if(status == 1) {
                        ...
                        if(c == 1) {
                            ...
                            // 设置 webView 为 nil，解决Webview调用reload导致JSContext失效的问题
...
- (void)timingTracking:(NSString *)cellphone andLon:(double)lon andLat:(double)lat andVehicleLocation:(NSString *)vehicleLocation  {
    ...
    NSString *url = [NSString stringWithFormat:@"%@%@", kApi, @"timingTracking.do"];
...
- (void)reverseGeo:(nullable NSString *)cellphone andLon:(double)lon andLat:(double)lat andWebView:(nullable WKWebView *)webView{
